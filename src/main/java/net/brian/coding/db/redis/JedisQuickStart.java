...
public class JedisQuickStart {
	JedisPool pool;
	Jedis jedis;
	int ONE_WEEK_IN_SECONDS = 7 * 86400;
	@Before
	public void setUp() {
	...
	/**
	 * 这个方法主要是通过java代码实现mdel的功能，redis原生的只有mget和mset，但是没有mdel
	 * 这个方法的实现原理就是利用Redis的Pipeline功能实现批量处理del命令
	 */
	@Test
	public void testMdel() {
		List<String> keys = Arrays.asList("a", "b", "c");
		Pipeline pipeline = jedis.pipelined();
		//pipeline执行命令，注意此时命令并未真正执行，而是被加入一个队列最终由sync提交批量执行
		for(String key : keys) {
			pipeline.del(key);
		}
		// 执行命令
	...
	public void testBasicString() {
		...
		// 2、直接覆盖原来的数据
		jedis.set("name", "brian");
		System.out.println(jedis.get("name"));// 执行结果：闵晓荣
		// 删除key对应的记录
		jedis.del("name");
		System.out.println(jedis.get("name"));// 执行结果：null
		/**
		 * mset相当于 jedis.set("name","minxr"); jedis.set("jarorwar","brian");
		 */
		jedis.mset("name", "minxr", "jarorwar", "brian");
